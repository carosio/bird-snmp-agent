...
ASN_COUNTER				= ASN_APPLICATION | 0x1
ASN_GAUGE				= ASN_APPLICATION | 0x2
	...
	def Table(self, entry, columns):
		...
		for column in columns:
			...
			for value in columns[column]:
				self.RegisterVar('%(ColumnName)s'% { 'ColumnName': column }, value)
				i += 1
...
class RequestObject(object):
	__slots__ = ['oid', 'mode', 'value', 'data', '__request', '__reqinfo']
	_re_ip = re.compile("([0-9]+\.[0-9]+\.[0-9]+\.[0-9]+)")
	_re_gauge = re.compile("^g\|(.*)")
	_re_counter = re.compile("^c\|(.*)")
	_re_octet = re.compile("^o\|(.*)")
	# class constructor
	def __init__(self, request, reqinfo):
	...
	def SetValue(self, value):
		...
		if not value_type:
			try: match = self._re_ip.search(value)
			except: match = None
			if match:
				value = socket.inet_aton(value)
				value_type = "ipaddr"
		if not value_type:
			try: match = self._re_counter.search(value)
			except: match = None
			if match:
				value = int(match.group(1))
				value_type = "counter"
		if not value_type:
			try: match = self._re_gauge.search(value)
			except: match = None
			if match:
				value = int(match.group(1))
				value_type = "gauge"
		if not value_type:
			try: match = self._re_octet.search(value)
			except: match = None
			if match:
				value = match.group(1)
				value_type = "octet"
...
def _handler_wrapper(handler, reginfo, reqinfo, requests):
	...
	while True:
		...
		if req.mode == SNMP_MSG_GET:
			if req.oid in axd:
				req.SetValue(axd[req.oid]['value'])
				# run read-write and read-only handlers
				for handler in request_handlers[PAX_RW] + request_handlers[PAX_RO]:
					value = handler(req)
					if value:
						req.SetValue(value)
		elif req.mode == SNMP_MSG_GETNEXT:
			if req.oid in axd:
				if axd[req.oid]['noid'] is None:
					# only set current objid
					req.SetNext(req.oid)
				else:
					# req.SetNext changes req.oid value
					req.SetNext(axd[req.oid]['noid'])
					req.SetValue(axd[req.oid]['value'])
					# run read-write and read-only handlers
					for handler in request_handlers[PAX_RW] + request_handlers[PAX_RO]:
						value = handler(req)
			...
			for handler in request_handlers[PAX_RW] + request_handlers[PAX_WO]:
				value = handler(req)
				if value:
					req.value = value
			# save value
			try:
				axd.Update(req.oid, req.value)
			except OperationalError:
				axd.RegisterVar(req.oid, req.value)
		if not r.next:
			break
		r = r.next.contents
...
class AgentX(object):
	...
	def Init(self, name, master=False, mibfile=(), rootoid=None):
		# setup log facility
		...
		axl.init_snmp(name)
		# register agent
		if not type(mibfile) in (list, tuple):
			mibfile = (mibfile,)
		for mib in mibfile:
			axl.read_mib(mib)
		...
		if rootoid:
			axd.container = rootoid.split('::', 1)[0]
	...
	def Register(self, handler, handler_type=PAX_RW):
		assert '__call__' in dir(handler), 'Callable object is required'
		request_handlers[handler_type].append(handler)
	...
	# infinite loop
	def MainLoop(self, interval, register_data_handler, *args):
		assert '__call__' in dir(register_data_handler), 'Callable object is required'
		if not self.loop:
			self.TimerStart(interval)
			self.loop = True
			while self.loop:
				axd.Clear()
				if args:
					register_data_handler(*args)
				else:
	...
	def Trap(self, oid, *args):
		...
		TrapOID		= ReadOID(axd.NormOID(oid))
